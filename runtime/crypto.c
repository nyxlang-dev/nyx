...
nyx_string* nyx_ec_p256_keypair(void) {
    ...
    memset(out, 0, 32);
    BN_bn2binpad(EC_KEY_get0_private_key(key), out, 32);
    size_t publen = EC_POINT_point2oct(EC_KEY_get0_group(key), EC_KEY_get0_public_key(key),
                                       POINT_CONVERSION_UNCOMPRESSED, out + 32, 65, NULL);
    EC_KEY_free(key);
    if (publen != 65) return nyx_string_from_cstr("");
    return nyx_string_from_ptr((const char*)out, 97);
}

// ECDH over P-256: 32-byte shared X-coordinate. priv=32B, peer_pub=65B uncompressed.
nyx_string* nyx_ecdh_p256(nyx_string* priv, nyx_string* peer_pub) {
    if (!priv || !peer_pub || priv->length < 32 || peer_pub->length < 65)
        return nyx_string_from_cstr("");
    EC_KEY* key = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1);
    if (!key) return nyx_string_from_cstr("");
    const EC_GROUP* grp = EC_KEY_get0_group(key);
    BIGNUM* bn = BN_bin2bn((const unsigned char*)priv->data, 32, NULL);
    EC_KEY_set_private_key(key, bn);
    EC_POINT* peer = EC_POINT_new(grp);
    unsigned char* shared = (unsigned char*)GC_MALLOC_ATOMIC(32);
    int ok = EC_POINT_oct2point(grp, peer, (const unsigned char*)peer_pub->data, 65, NULL) == 1;
    int r = ok ? ECDH_compute_key(shared, 32, peer, key, NULL) : -1;
    BN_free(bn); EC_POINT_free(peer); EC_KEY_free(key);
    if (r != 32) return nyx_string_from_cstr("");
    return nyx_string_from_ptr((const char*)shared, 32);
}

// ECDSA/P-256 signature over a 32-byte digest → raw r‖s (64 bytes, ES256).
nyx_string* nyx_ecdsa_p256_sign(nyx_string* priv, nyx_string* hash) {
    if (!priv || !hash || priv->length < 32 || hash->length < 32)
        return nyx_string_from_cstr("");
    EC_KEY* key = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1);
    if (!key) return nyx_string_from_cstr("");
    BIGNUM* bn = BN_bin2bn((const unsigned char*)priv->data, 32, NULL);
    EC_KEY_set_private_key(key, bn);
    ECDSA_SIG* sig = ECDSA_do_sign((const unsigned char*)hash->data, 32, key);
    if (!sig) { BN_free(bn); EC_KEY_free(key); return nyx_string_from_cstr(""); }
    ...
    ECDSA_SIG_free(sig); BN_free(bn); EC_KEY_free(key);
    return nyx_string_from_ptr((const char*)out, 64);
}

// Verify a raw r‖s (64B) signature over a 32-byte digest with a 65B public key.
int64_t nyx_ecdsa_p256_verify(nyx_string* pub, nyx_string* hash, nyx_string* sig) {
    if (!pub || !hash || !sig || pub->length < 65 || hash->length < 32 || sig->length < 64)
        return 0;
    EC_KEY* key = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1);
    if (!key) return 0;
    const EC_GROUP* grp = EC_KEY_get0_group(key);
    ...
    int64_t result = 0;
    if (EC_POINT_oct2point(grp, pt, (const unsigned char*)pub->data, 65, NULL) == 1 &&
        EC_KEY_set_public_key(key, pt) == 1) {
        ECDSA_SIG* s = ECDSA_SIG_new();
        BIGNUM* rr = BN_bin2bn((const unsigned char*)sig->data, 32, NULL);
        BIGNUM* ss = BN_bin2bn((const unsigned char*)sig->data + 32, 32, NULL);
        ECDSA_SIG_set0(s, rr, ss); // takes ownership of rr, ss
        result = (ECDSA_do_verify((const unsigned char*)hash->data, 32, s, key) == 1) ? 1 : 0;
        ECDSA_SIG_free(s);
    }
    EC_POINT_free(pt); EC_KEY_free(key);
    return result;
}

// HKDF-SHA256 (RFC 5869, extract+expand). Empty string on error / len<=0.
nyx_string* nyx_hkdf_sha256(nyx_string* salt, nyx_string* ikm, nyx_string* info, int64_t len) {
    if (len <= 0 || !ikm) return nyx_string_from_cstr("");
    unsigned char* okm = (unsigned char*)GC_MALLOC_ATOMIC((size_t)len);
    ...
    int ok = pctx
        ...
        && EVP_PKEY_CTX_set1_hkdf_salt(pctx,
             (const unsigned char*)(salt ? salt->data : ""), salt ? (int)salt->length : 0) == 1
        && EVP_PKEY_CTX_set1_hkdf_key(pctx,
             (const unsigned char*)ikm->data, (int)ikm->length) == 1
        && EVP_PKEY_CTX_add1_hkdf_info(pctx,
             (const unsigned char*)(info ? info->data : ""), info ? (int)info->length : 0) == 1
        && EVP_PKEY_derive(pctx, okm, &outlen) == 1;
    if (pctx) EVP_PKEY_CTX_free(pctx);
    if (!ok) return nyx_string_from_cstr("");
    return nyx_string_from_ptr((const char*)okm, len);
}

// AES-128-GCM: encrypt pt with key(16B)+iv(12B)+aad → ciphertext ‖ 16B tag.
nyx_string* nyx_aes128gcm_encrypt(nyx_string* key, nyx_string* iv,
                                  nyx_string* pt, nyx_string* aad) {
    if (!key || !iv || !pt || key->length < 16 || iv->length < 12)
        return nyx_string_from_cstr("");
    int ptlen = (int)pt->length;
    unsigned char* out = (unsigned char*)GC_MALLOC_ATOMIC((size_t)ptlen + 16);
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    int outl = 0, total = 0, ok = 0;
    if (ctx
        ...
        if (aad && aad->length > 0)
            EVP_EncryptUpdate(ctx, NULL, &outl, (const unsigned char*)aad->data, (int)aad->length);
        if (EVP_EncryptUpdate(ctx, out, &outl, (const unsigned char*)pt->data, ptlen) == 1) {
            total = outl;
            if (EVP_EncryptFinal_ex(ctx, out + total, &outl) == 1) {
                total += outl;
                ok = EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, out + total) == 1;
            }
        }
    }
