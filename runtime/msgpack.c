...
nyx_string* nyx_msgpack_unpack_str(nyx_string* data) {
    ...
    // str_len y el offset de cabecera vienen del BLOB (peer). Validar que los
    // bytes anunciados existen de verdad: sin esto, memcpy lee heap ajeno y lo
    // devuelve como String (divulgación de memoria, no solo crash).
    size_t header_len = (size_t)(str_start - (const char*)p);
    if (data->length < 0 || (size_t)data->length < header_len)
        return nyx_string_from_cstr("");
    if (str_len > (size_t)data->length - header_len)
        return nyx_string_from_cstr("");
