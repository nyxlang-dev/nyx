...
nyx_string* nyx_ws_parse_frame(nyx_string* data) {
    ...
    // El peer controla payload_len (hasta 2^64-1). Validar SIN sumar: una suma
    // header_size + payload_len envuelve y deja pasar longitudes absurdas
    // (14 + (2^64-1) == 13), y el cast a int64_t de un valor con el bit alto
    // puesto da negativo, que también pasaba el guard viejo.
    if (data->length < 0 || (size_t)data->length < header_size)
        return nyx_string_from_cstr("");
    size_t available = (size_t)data->length - header_size;
