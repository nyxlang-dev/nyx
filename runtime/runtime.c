...
nyx_array_t* nyx_array_new_ptr(void) {
    ...
    arr->length = 0;
    arr->capacity = 8;
    arr->data = (int64_t*)GC_MALLOC(sizeof(int64_t) * arr->capacity);
    return arr;
}
...
void nyx_array_push_ptr(nyx_array_t* arr, void* value) {
    if (arr->length >= arr->capacity) {
        arr->capacity *= 2;
        int64_t* new_data = GC_MALLOC(sizeof(int64_t) * arr->capacity);
        memcpy(new_data, arr->data, sizeof(int64_t) * arr->length);
        arr->data = new_data;
    }
    arr->data[arr->length++] = (int64_t)(uintptr_t)value;
}
...
void nyx_array_set_ptr(nyx_array_t* arr, int64_t index, void* value) {
    nyx_bounds_check(index, arr->length);
    arr->data[index] = (int64_t)(uintptr_t)value;
}

// ===== PROCESS/OS FUNCTIONS (v5.2) =====

// Execute a shell command via /bin/sh. Returns exit code.
//
// SECURITY CONTRACT: this is a deliberate shell-exec primitive (like
// Python's os.system). `cmd` is passed to the shell verbatim, so the CALLER
// must never feed it untrusted/unsanitized input — doing so is command
// injection. For untrusted arguments use the array-based process builtins
// (fork + execvp, see runtime/process.c), which do not invoke a shell.
#ifndef __wasi__
...
#else
// WASI: no hay shell ni procesos — exec() devuelve -1 (error).
// EN: no shell/processes on WASI — exec() returns -1 (error).
int64_t nyx_exec(const char* cmd) { (void)cmd; return -1; }
#endif
