...
// Async timer: fire callback once after delay_ms (approximate), then the
// timer deactivates itself (one-shot only — call add_timer again to repeat).
// The timeout passed to run_once() is shortened as needed so the soonest
// active timer fires on time even if the caller's timeout is longer/-1.
