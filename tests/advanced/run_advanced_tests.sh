...
for test_file in tests/advanced/test-A*.nx; do
    ...
    if make compile FILE="$test_file" > /dev/null 2>&1; then
        ...
        if clang script.ll runtime/runtime.c runtime/strings.c runtime/runtime-arrays.c \
            runtime/maps.c runtime/file-io.c runtime/iterators.c runtime/net.c runtime/thread.c runtime/regex.c runtime/time.c runtime/crypto.c runtime/tls.c -lgc -lpthread -ldl -lm -lssl -lcrypto -o /tmp/nyx_adv_test 2>/dev/null; then
            # Run with timeout
