...
# Ej:  bash scripts/build-release.sh 0.13.0 x86_64
...
# 2. Crear estructura de release (layout idéntico a un clone)
mkdir -p "$DIST/$NAME/scripts"
mkdir -p "$DIST/$NAME/compiler"
...
# Binarios compilados (en raíz, como después de make bootstrap)
cp nyx_bootstrap "$DIST/$NAME/nyx_bootstrap"
cp nyx_build "$DIST/$NAME/nyx_build"

# Wrapper script (en scripts/, como en el repo)
cp scripts/nyx "$DIST/$NAME/scripts/nyx"
chmod +x "$DIST/$NAME/scripts/nyx"

# Compiler sources (build.nx necesario para recompilar nyx_build)
cp compiler/build.nx "$DIST/$NAME/compiler/"
cp compiler/*.ll "$DIST/$NAME/compiler/"

# Makefile (necesario para make build-nyx-build, make bootstrap, etc.)
