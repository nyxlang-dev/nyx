...
# trait con supertrait + método sin body + default method CON body + assoc
# type — fmt_trait leía d[1] (supertraits) como si fueran las firmas
# (índice desfasado, d[2] son las firmas reales) y perdía bodies/assoc types.
check_fixture "trait-assoc-default" "tests/fmt/trait-assoc-default.nx" \
    "trait Shape: Printable {" "type Item" "return \"shape\""

# impl Trait for Type con `type Item = int` — fmt_impl_trait nunca emitía
# d[3] (assoc_types).
check_fixture "impl-trait-assoc" "tests/fmt/impl-trait-assoc.nx" \
    "type Item = int"
