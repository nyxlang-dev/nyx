...
> **Note**: Current version is **v0.12.0** (pre-release). Versions v0.2.0-v5.4.0 were assigned during rapid development before a formal versioning policy was adopted. The version was reset to v0.9.0 and then progressed through v0.10.0-v0.12.0 to reach v0.12.0. See [VERSIONING.md](VERSIONING.md) for details.

---

## [REVERTED] — v1.0.0 was declared prematurely, reverted to v0.12.0

> This entry was auto-generated by Claude Code without author authorization. The version has been reverted. See VERSIONING.md.
...
- Comprehensive SPEC.md verified against implementation
- Stress tests passing all 6 levels

---

## [Unreleased] — Products & Compiler Fixes

### Added
- **nyx-kv** (`products/kv/`) — Redis-compatible key-value store
  - 15 RESP2 commands: PING, SET, GET, DEL, EXISTS, KEYS, EXPIRE, TTL, INCR, DECR, MSET, MGET, DBSIZE, FLUSHDB, INFO
  - C-level RESP parser (`nyx_resp_read_command` in runtime/net.c)
  - Zero-alloc GET response via `resp_write_bulk` (writev)
  - 128 workers, channel-based fd dispatch, port 6380
  - Performance: 49K ops/s default, 161K ops/s pipelined
- **nyx-serve** (`products/serve/`) — HTTP framework for serving web applications
  ...
  - `main.nx` — Example app serving nyxlang.com landing page on port 3000
- **nyx-proxy** (`products/proxy/`) — Multi-threaded HTTP reverse proxy
  - TOML-based configuration (`proxy.toml`) with numbered upstream sections
  - Path prefix routing with longest-prefix match
  - Reuses `forward_request` and `select_backend` from std/proxy.nx
  - 64 workers via http_serve_mt, 150KB binary
- **Map.remove(key)** and **Map.clear()** — new HashMap methods (runtime/maps.c + codegen)
- Test 193: test-193-map-remove-clear

### Fixed
- **Compiler: global struct types** — Pre-pass registers struct names before global variable pass, so `nyx_type_to_llvm` resolves struct types correctly instead of defaulting to `i64`
- **Compiler: zeroinitializer for struct globals** — Struct-typed globals now use `zeroinitializer` instead of `0`, preventing memory corruption at -O2
- **Compiler: Fn return type in indirect calls** — `Fn(T) -> R` annotations now propagate the return type to indirect call IR, fixing -O2 crash when handler returns a struct
- **nyx-kv: charAt() comparisons** — Fixed RESP parser comparing charAt() results (int) with string literals instead of ASCII values
...
### Changed
- Version reset from v5.4.0 to v0.9.0 (see VERSIONING.md for rationale)
- All documentation updated to reflect v0.9.0

### Added
- Feature audit (docs/AUDIT_PHASE1.md) — 136 PASS, 8 PARTIAL, 14 FAIL, 5 DECORATIVE
...
- ROADMAP.md: actualizado completamente con tabla de gaps y timeline v1.8.0-v3.0.0
