# Nyx v0.9.0 — Language Comparison

> Honest comparison based on Phase 1 audit (2026-03-24).
...
- No functional package registry (nyx.toml exists but no repos)
- No complete public documentation (SPEC outdated)
- Tools exist (fmt, vet, doc, check) but no external adoption
...
- Complete JSON parser/serializer
- However: no HTTP server tested under load, no TLS server, no proxy
- No sandboxing, no end-to-end template engine
- Pieces exist but aren't integrated or tested as a stack
...
- No tutorial, no playground, no community support
...
### Honest weaknesses:
1. **Nonexistent ecosystem** — this is problem #1
...
5. **Inconsistent documentation** — SPEC outdated, contradictory versions
6. **No benchmarks** — performance claims unverified
...
3. At least one real benchmark
4. Real async/await or honestly document it's syntactic
5. At least one working end-to-end full-stack use case
