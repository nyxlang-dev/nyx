...
Fields: `code` (stable identifier, see catalog), `severity` (always `error` today),
`phase` (`parse` or `semantic`), `line`/`column` (1-based position in the ORIGINAL source file —
...
`severity` (hoy siempre `error`), `phase` (`parse` o `semantic`), `line`/`column` (posición 1-based en el
