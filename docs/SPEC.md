...
| NYX1037 | calling a value declared bare `Fn` where a struct-by-value or a `float` is expected — a `Fn` without a signature carries no return convention, so the call assumes an integer and the program crashes (struct) or returns garbage (float) at run time. `int`, `String`, `bool`, `char`, `Result`, `Option` and a discarded result are unaffected: for those the assumed convention matches. Declare the signature: `f: Fn(T) -> R` |
| NYX1038 | `for … in` over a `String` — for-in walks `Array`, `Map`, ranges and iterators only. It used to compile and the binary died with SIGSEGV and no location (the string was read as an array). Iterate by index: bytes with `0..s.length()` + `substring(i, i + 1)`, UTF-8 characters with `0..s.char_length()` + `char_substring(i, i + 1)`. Reported by `nyx check` and, with semantic off, by codegen |
...
| NYX2018 | RUNTIME slot-type mismatch, Array side: an Array slot whose runtime tag is `int`, `String`, `float` or `bool` walked as `Array` by `for x: Array in xs` — it used to crash later, in the inner loop, far from the culprit. Untagged and opaque slots pass as before; `NYX_SLOT_CHECK=off` disables it |
