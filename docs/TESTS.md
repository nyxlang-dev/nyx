...
|-------|---------|--------|------|
| Regression | `make test` | **441 archivos / 440 ARM64** | `test-123-full-asm` se salta en ARM64 (arquitectura). Altas del 2026-09-08 al 13: +28 (detalle por test en `CHANGELOG.md`) |
...
| Runtime C unit (B4) | `make test-runtime` | **34 suites / 1665 asserts** | verificado con corrida real 2026-09-13; +5 de `test_net_result` al sacar sus puertos fijos del rango efímero (fallaba 5 de 5 con otra conexión abierta en el equipo) |
...
| Puertas de tipos del tooling | (dentro de `make test-ai-first`) `run_tooling_gates.sh` | **7 checks** (4 negativos + 3 positivos) | `nyx check`/`nyx test`/`nyx build` acusan el error de tipos y salen limpios sobre un proyecto correcto; ejercen `src/`, prelude, `std/` y el compile desde otro cwd |
...
| WASM (wasm32-wasi) | `make test-wasm` | **28** | SKIP limpio sin toolchain; +4 el 2026-09-10 (proyecto por el CLI que compara nativo vs wasm; lifetime de cierres: captura de String, dos listeners, y anclaje sin fuga); +1 el 2026-09-13 (SSE y `browser_geo_fn`, con `fetch` falso en streaming) |
