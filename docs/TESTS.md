...
|-------|---------|--------|------|
| Regression | `make test` | **443 archivos / 442 ARM64** | `test-123-full-asm` se salta en ARM64 (arquitectura). Altas del 2026-09-08 al 14: +30 (detalle por test en `CHANGELOG.md`) |
...
| AI-first (objetivo) | `make test-ai-first` | **26 programas + 47 guardas** | 25 scripts: `run_ai_first_tests.sh` + 23 de guardas (47: `run_templates_parity.sh` 2, `run_tooling_gates.sh` 10, `run_build_manifest.sh` 14, `run_no_compiler_rt_builtins.sh` 2, voseo 1) + `scripts/sdd/selftest` |
...
| Puertas de tipos del tooling | (dentro de `make test-ai-first`) `run_tooling_gates.sh` | **10 checks** (6 negativos + 4 positivos) | `nyx check`/`nyx test`/`nyx build` acusan el error de tipos y salen limpios sobre un proyecto correcto; +3 (2026-09-14): opción desconocida de `nyx test` da rc!=0 |
| Manifiesto de `nyx build` | (dentro de `make test-ai-first`) `run_build_manifest.sh` | **14 checks** (9 negativos + 5 positivos) | `[build]` (`target`, `main`, clave desconocida, contradicción) y `--main` (errores previos; no pisa el principal; `target/` cubierto por el `.gitignore` sembrado) |
...
| WASM (wasm32-wasi) | `make test-wasm` | **31** | SKIP sin toolchain (salvo los guards); +3 el 2026-09-14 (`--main`; guard con archivo:línea; guard de concurrencia); +1 el 09-13 (SSE); +4 el 09-10 (CLI; cierres). Detalle en `CHANGELOG.md` |
