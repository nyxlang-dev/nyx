...
| Recetas by-example | `make test-examples` | **127** (110 ejecutan, 17 solo compilan+enlazan) | +8 desde el 09-23 (`113..120`; 114 y 117 solo wasm); compila, ENLAZA y CORRE cada receta con exit 0; motivo en el script |
...
| Integration E2E | `make test-integration` | **11 sub-suites** (keep-alive de serve 13 + WS proxy 6 + FFI 3 + slots 9 + llm 3 + HTTP/2 1 + body cap 6 + serve contrato 10 + serve bind 6-9 + smoke 103 + serve+kv 10) | detalle de las altas en `CHANGELOG.md` |
