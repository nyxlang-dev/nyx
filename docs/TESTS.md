...
| REPL / intérprete | `make test-repl` | `scripts/testing/run_repl_smoke.sh` | **3 checks** (v0.24.2, cierre de la brecha silently-wrong del intérprete: método fuera del subconjunto declarado → NYX3001 ruidoso; la sesión del REPL SOBREVIVE al error; control negativo `s.length()`==4. RED verificado contra el binario previo. El target construye nyx_repl SIEMPRE antes del smoke — contra un binario stale certificaría el pasado). Dentro de `make test-all` | ✅ |
